...
import { ProjectVehicleData } from './project.interface';
...
@Injectable()
export class ProjectVehicleService {
  constructor(
    @InjectRepository(ProjectVehicleEntity)
    private readonly repository: Repository<ProjectVehicleEntity>,
    private readonly userService: UserService
  ) {}

  async findAll(token?: string): Promise<Array<ProjectVehicleData>> {
    if (token) {
    ...
    return await this.repository
      .find()
      .then(data => data.map((el: ProjectVehicleEntity) => this.buildDataRO(el)));
  }

  async findById(id: string): Promise<ProjectVehicleData> {
  ...
  async create(dto: CreateDto, token: string): Promise<ProjectVehicleData> {
    ...
    const newEntity = new ProjectVehicleEntity();
  ...
  private buildDataRO(entity: ProjectVehicleEntity): ProjectVehicleData {
    return {
      _id: entity._id,
      name: entity.name,
