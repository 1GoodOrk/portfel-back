...
import { ProjectAutoData } from './project.interface';
...
@Injectable()
export class ProjectAutoService {
  constructor(
    @InjectRepository(ProjectAutoEntity)
    private readonly repository: Repository<ProjectAutoEntity>,
  ...
  async findAll(token?: string): Promise<any> {
    ...
    return await this.repository
      .find()
      .then(data => data.map((el: ProjectAutoEntity) => this.buildDataRO(el)));
  }
  ...
  async create(dto: CreateDto, token: string): Promise<any> {
    ...
    const newEntity = new ProjectAutoEntity();
