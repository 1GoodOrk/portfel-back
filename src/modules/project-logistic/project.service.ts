...
import { ProjectLogisticData } from './project.interface';
...
@Injectable()
export class ProjectLogisticService {
  constructor(
    @InjectRepository(ProjectLogisticEntity)
    private readonly repository: Repository<ProjectLogisticEntity>,
  ...
  async findAll(token?: string): Promise<any> {
    ...
    return await this.repository
      .find()
      .then(data => data.map((el: ProjectLogisticEntity) => this.buildDataRO(el)));
  }
  ...
  async create(dto: CreateDto, token: string): Promise<any> {
    ...
    const newEntity = new ProjectLogisticEntity();
